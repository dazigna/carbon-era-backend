...
class AdemeParser():
  ...
  def parse(self):
    ...
    self.carbdonDf = dataFrameClean.loc[
    ...
    # Create units
    dfUnits = pd.DataFrame(self.carbdonDf['unite_francais'].unique(), columns=['name'])
    self.units = self.createUnits(dfUnits)

    #Create categories
    dataFrameCategories = pd.DataFrame(self.carbdonDf['code_de_la_categorie'].unique(), columns=['categories'])
    self.categories = self.createCategories(dataFrameCategories)

  def writeToFiles(self):
    print(f'Writing data to files ...')
    # JSON Writing
    carbonJson = self.carbdonDf.to_json(orient='records', indent=4)
    with open('carbonDatabase.json', 'w', encoding='utf-8') as f:
      f.write(carbonJson)

    unitJson = self.units.to_json(orient='records', indent=4)
    with open('carbonUnits.json', 'w', encoding='utf-8') as f:
      f.write(unitJson)

    with open('carbonEnums.json', 'w', encoding='utf-8') as f:
      json.dump(self.carbonEnums, f, indent=4)

    with open('carbonCategories.json', 'w', encoding='utf-8') as f:
